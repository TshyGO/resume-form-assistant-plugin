...
(function attachResumeProProfile(globalScope) {
  ...
  const PROFILE_SCHEMA = [
    {
      ...
      fields: [
        { id: "name", key: "姓名" },
        { id: "namePinyin", key: "姓名拼音", placeholder: "ZHANG SAN" },
        { id: "usedName", key: "曾用名" },
    ...
    },
    {
      ...
      fields: [
        ...
        { id: "homeAddress", key: "家庭住址", type: "textarea" },
        { id: "currentAddress", key: "现居住地", type: "textarea" }
      ]
    },
    {
      name: "联系方式",
      fields: [
        { id: "phone", key: "手机号码" },
        { id: "email", key: "常用邮箱" },
        { id: "emergencyName", key: "紧急联系人姓名" },
        { id: "emergencyRelation", key: "紧急联系人关系", placeholder: "父亲" },
        { id: "emergencyPhone", key: "紧急联系人电话" }
      ]
    },
    {
      name: "教育补充",
      fields: [
        { id: "highestEducation", key: "最高学历", type: "select", options: ["博士研究生", "硕士研究生", "大学本科", "大学专科", "高中及以下"] },
        { id: "highestDegree", key: "最高学位", type: "select", options: ["博士", "硕士", "学士", "无"] },
        { id: "studyMode", key: "学习形式", type: "select", options: ["全日制", "非全日制"] },
        { id: "graduation", key: "毕业时间", type: "month" },
    ...
    },
    {
      name: "语言与技能",
      fields: [
        { id: "language", key: "外语语种", placeholder: "英语" },
  ...
  const SECRET_LABEL = /密码|口令|验证码|校验码|password|captcha/i;
  ...
  function normalizeProfile(raw) {
    ...
    for (const item of Array.isArray(source.custom) ? source.custom : []) {
      ...
      if (!normalized || seen.has(normalized) || profile.custom.length >= MAX_CUSTOM_FIELDS) continue;
      seen.add(normalized);
      profile.custom.push({ key, value: text(item?.value) });
    }
  ...
  function profileToResumeFields(rawProfile) {
    ...
    // 同一关系有多人时加序号（兄弟姐妹1姓名、兄弟姐妹2姓名），只有一人时不加。
    const totals = {};
    ...
    profile.family.forEach((member) => {
      ...
      const prefix = totals[member.relation] > 1 ? `${member.relation}${seen[member.relation]}` : member.relation;

      FAMILY_FIELDS.forEach((field) => {
        if (member[field.id]) fields.push({ group: FAMILY_GROUP, key: `${prefix}${field.key}`, value: member[field.id] });
      });
    });

    profile.custom.forEach((item) => {
      if (item.value) fields.push({ group: CUSTOM_GROUP, key: item.key, value: item.value });
    });
  ...
  // 已经有着落的字段名：预置字段（字段名和界面标签）、家庭成员生成的字段名、补充字段、模板字段。
  function knownFieldKeys(rawProfile, resumeFields) {
    ...
    profileToResumeFields(profile).forEach((field) => add(field.key));
  ...
  function addPendingFields(rawProfile, labels) {
    const profile = normalizeProfile(rawProfile);
    const known = knownFieldKeys(profile, []);
    let added = 0;

    for (const label of Array.isArray(labels) ? labels : []) {
      const key = text(label);
      const normalized = normalizeKey(key);
      if (!normalized || known.has(normalized) || profile.custom.length >= MAX_CUSTOM_FIELDS) continue;
  ...
  function mergeProfiles(localProfile, incomingProfile) {
    ...
    const family = local.family.length ? local.family : incoming.family;
