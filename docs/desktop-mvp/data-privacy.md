...
| 内容 | 表单字段描述 + 相关简历分组（填写）；岗位识别另见 §8.4，只发编号片段 | 证据正文/片段 + **最少**候选申请元数据 |
| 确认 | 用户主动点「一键 AI 填写」即同意该次填写外发。点「保存岗位到桌面端」且本地提取不可靠时，才可能发起一次岗位识别，并先展示服务商与片段。识别结果要等用户在表单里点确认才写入桌面 | 发送前预览范围、服务商、模型 |
...
本地规则已经得到可靠的公司和岗位时，不调用模型。只有公司或岗位缺失、两者疑似填反、候选只是站点标题，或本地候选互相冲突时，才使用 `chrome.storage.local.aiConfig` 的 `apiUrl` / `model` / `apiKey`。请求走现有 offscreen 进程，不读取桌面那一套 Key，也不把插件 Key 复制到桌面。

一次请求的用户内容只有编号片段：`id`、来源标签、角色、短文本。角色限于公司标识、职位标题、招聘元数据里的岗位工作地点、页面标题。每段不超过 160 字，最多 8 段。页面链接仍由本地 URL 脱敏规则生成，不放进请求，也不交给模型编写。

不发送：整页 HTML 或正文、投递表单里已填写的姓名 / 电话 / 邮箱 / 证件号、简历和「我的信息」、Cookie、凭据、带查询参数的原始 URL。意向工作地点、期望工作地点、面试站点不作为岗位地点，也不会因为模型提到它们就被保存。

模型只能返回 `company` / `title` / `location` 和对应片段编号。字段必须与所引用片段的全文（职位标题可以是「你正在投递职位」后面的全文）一致；独立、完整、以「公司」或「集团」结尾的页面标题也可作为公司证据，含招聘或岗位文字的标题不能当公司。对不上就丢弃。无配置、无片段、无证据、格式错误、网络失败、超时或用户取消时，打开可编辑表单，不自动再请求，也不静默保存猜出来的值。保存按钮旁在点击前说明本地识别不准时可能外发岗位片段；岗位识别请求不跟随重定向。
