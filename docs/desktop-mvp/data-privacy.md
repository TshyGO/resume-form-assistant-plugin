...
| 插件 `aiConfig` 与 `aiProfiles` | 用户 | 仅扩展存储（今天明文）。`aiConfig` 是当前这一份的镜像 | **禁止**把其中的 Key 复制到桌面档案或桌面备份 |
| 桌面模型 Key（每份配置一条） | 用户 | OS 凭据库：Windows Credential Manager / DPAPI；macOS Keychain。账户名按配置 id 分开，升级前的固定账户 `ai-api-key` 会迁到对应配置后不再当作回退 | **禁止**进 SQLite、附件、备份、日志、`ai-settings.json` 和命令返回值 |
...
插件弹窗的「导出备份」可以写入每一份 AI 配置自己的 `apiKey`，四个条件必须同时满足：

1. 默认不含任何一份的 Key。用户每次都要主动勾选「包含 API Key」，勾选状态不落盘。
2. 勾选后导出前再确认一次。确认文案要说明文件是明文、不要分享，并说明插件不收集也不上传数据。
3. 只限插件自己的 AI API Key，而且每把 Key 只和导出它的那一份配置、那个地址写在一起。恢复时不能把 A 的 Key 配到 B 的地址上。桌面端凭据、Cookie、`Authorization`、OTP、密码一律不适用。
4. 只限用户点按钮触发的这一次下载。自动备份、桌面档案库备份、事件、快照、日志、诊断导出仍然一个都不许有。

多份配置写在备份的 `aiProfiles`（格式版本 3）。只有一份时仍沿用原来的 `aiConfig`，旧备份可以导入。桌面备份继续排除 `ai-settings.json` 和全部 Key。
...
| | 插件填写/解析 | 桌面通知整理（D11） |
| --- | --- | --- |
| 配置位置 | `chrome.storage.local` 的 `aiProfiles` 与当前镜像 `aiConfig`（今天明文） | `ai-settings.json` 只存地址、模型名和当前 id。每份 Key 在 Windows Credential Manager / DPAPI 或 macOS Keychain 的独立账户 |
| 发往 | 当前这份的 `apiUrl`；点「获取模型」时另向同一服务的 `/models` 发一次只带这一份 Key 的 GET。非本机 `http://` 时设置页提示明文风险（局域网地址温和提示，公网地址明确警告），不阻止使用 | 当前这份桌面配置的接口。获取模型同样只带这一次输入的 Key，或这一份已保存且地址没变的 Key |
...
Key 只在 `Authorization` 头里，不进正文、不进预览、不进日志、不进备份（`ai-settings.json` 也不含 Key，见 §1 的排除表）。切换配置时只读取当前这份的 Key；这一份没有 Key 就明确失败，不会改用其他配置或升级前那条固定凭据。获取模型列表本身不改已选模型，也不保存还没点「保存并使用」的输入。
