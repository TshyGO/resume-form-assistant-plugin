...
不发送：整页 HTML 或正文、投递表单里已填写的姓名 / 电话 / 邮箱 / 证件号、简历和「我的信息」、Cookie、凭据、带查询参数的原始 URL。意向工作地点、期望工作地点、面试站点不算岗位地点，模型提到了也不会保存。
...
桌面不可用、AI 未配置、超时（AI 请求等 25 秒；Worker 本身没反应时 30 秒后放弃）、取消、格式错误或没有证据时，打开可编辑表单并说明原因，不自动再请求，也不静默保存猜出来的值。AI 未配置时表单里多一个「打开桌面 AI 设置」。桌面版本太旧、还不认识这种请求时，提示更新桌面。
