...
AI 配置片的 apiKey 是协议唯一允许携带 Key 的位置。桌面接收时将 Key 放入 OS 凭据库临时账户 ai-api-key:import-<importId>，SQLite 只存去 Key 后的 apiUrl、model 和 hasKey 标记。确认成功后 Key 移到正式服务商账户并清除临时账户；拒绝或过期也清除。模板与档案已写入、只差 AI 配置（例如临时 Key 丢失或服务商已满）的批次，用户拒绝时返回 rejected，保留已写入的模板与档案，也保留插件旧副本和旧 Key，并清除桌面临时账户；批次完成后重发的 AI 配置片不会再把 Key 放回凭据库。临时账户删除失败时记下待清理，下次启动重试。凭据库失败时不把该片标为已收齐，确认失败可重试且不会重复导入模板。Key 不得进入日志、遥测、SQLite、桌面备份或诊断导出。插件只有看到 imported 才能清除旧副本。

对已应用模板与档案、尚未完成 AI 确认的批次，拒绝操作会先清理可能已写入的正式服务商及其 Key；清理失败时保持待确认状态，供用户重试。只有清理成功才向插件返回 rejected。
