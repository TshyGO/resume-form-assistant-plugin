...
旧版插件与桌面曾分别保存 Key；PR 4 插件接入后统一使用桌面「当前使用」服务商，旧插件 Key 分片先进入 OS 凭据库临时账户，确认后才移入正式账户：

| | 插件填写/解析 | 桌面通知整理（D11） |
| --- | --- | --- |
| 配置位置 | 旧版 `chrome.storage.local.aiConfig`；PR 4 接入后配置与 Key 由桌面管理，插件不保留 Key | Windows：Credential Manager / DPAPI；macOS：Keychain；每个服务商一条（账户 `ai-api-key:<服务商 id>`） |
| 发往 | 经桌面转发，发往桌面「当前使用」的服务商；插件不持有 Key（PR 4 接入后生效）。旧版插件仍按原配置发往用户填写的 apiUrl | 用户在桌面设置的「当前使用」服务商；设置页点「获取模型」时另向所填地址对应的 /models 发一次只带 Key 的 GET（Key 优先用刚填的；否则只有地址与已保存地址同源——协议、主机、端口都一致——时才用该服务商已存的 Key。15 秒超时，日志只记主机名，报错只按状态码分类、不回显服务商正文，失败不拦手填）。编辑服务商时接口地址换了协议或主机，该服务商的 Key 自动清除。获取模型属于用户主动的配置动作，只带 Key、不带任何简历或证据内容，因此不走发送前预览。 |
| 内容 | 表单字段描述 + 相关简历分组（现有逻辑） | 证据正文/片段 + **最少**候选申请元数据；简历解析：用户上传的简历全文（发送前显示发给哪个服务商、主机、模型与字数，用户点「发送并解析」才发出） |
...
插件 Key **永不**因「桌面也要用 AI」而被复制。用户若在桌面再配一次，那是第二条凭据。唯一的例外是旧数据导入（§7.4）：用户升级插件时，旧 AI 配置片里的 Key 只进 OS 凭据库的临时账户，用户在桌面确认后移到新建服务商的账户并删除临时账户；它从不进入 SQLite、桌面备份、日志或诊断导出。
