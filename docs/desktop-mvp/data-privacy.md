...
不发送：整页 HTML 或正文、投递表单里已填写的姓名 / 电话 / 邮箱 / 证件号、简历和「我的信息」、Cookie、凭据、带查询参数的原始 URL。片段文字里只要像链接（带 `://` 或查询串）或像凭据（`token=`、`api_key:` 这类），整段不发。意向工作地点、期望工作地点、面试站点不算岗位地点，模型提到了也不会保存。
