...
### 5.0 从 0.4.0 升级到 0.4.1（插件数据迁到桌面）

0.4.1 起简历模板、「我的信息」和 AI 设置只在桌面里，插件只负责填写。0.4.0 插件里的数据会在第一次连上桌面时自动迁移：

1. 先装好桌面 0.4.1 并完成浏览器连接（§4）。**先装桌面、再升级插件**；商店版插件自动升级后若还没装桌面，侧栏会提示去下载，旧数据留在插件里不会丢。
2. 插件发现旧数据后发给桌面，并把桌面切到「简历」页。页面顶部列出要导入的模板名与字段数、「我的信息」项数、AI 服务的主机名与模型。
3. 点「导入到桌面」。桌面已经有「我的信息」时要先选保留哪份；模板加起来超过 25 个时先删掉一些再导入。
4. 导入完成后插件清掉自己那份旧数据。点「不导入」则插件数据原样保留，可以在插件状态页（侧栏底部「插件状态」）重新发送。

没迁过去的东西都留在插件状态页：超过 24 KB 或超过 25 个的模板可以下载 CSV 再到「简历」页导入；桌面没导入的旧 API Key 可以复制或删除；「我的信息」里像密码、验证码的项按规则不迁移。
